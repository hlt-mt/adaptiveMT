...
class Decoder_Moses_nbest:
        ...
        and handles communication, i.e. sending input and parsing output.
        """
	def __init__(self, parser, Tuning=None):
                """
                Start a new decoder with the specified command line arguments.
                """
                self.parser = parser
		self.tuning_obj = Tuning
		self.checkweights=0
                # set default parameters (if needed)
                default_decoder_options = "-xml-input inclusive"
                #default_decoder_options = default_decoder_options + " -use-persistent-cache false"
...
#pattern to match in order to know that the output of the show-weights has been produced
		...
		self.weightontuning_signal_pattern = re.compile("^\tweight:")
		...
		if self.parser.get('tuning', 'switch') == "on" and self.checkweights==0:
		    self.checkweights=1
		    while True:
			line = self.decoder.stdout.readline().strip()
			if "weight:" in line:
			    line = re.sub(r"^\s*weight:","",line)
			    self.tuning_obj.read_init_features(line)
			    break
