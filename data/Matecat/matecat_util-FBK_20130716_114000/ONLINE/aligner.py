...
class Aligner_Constrained_Search:
	...
	def align(self, source="", target="", correction="", moses_translation_options=""):
		...
		rr = str(random.randint(0,1000000))
                crt_file = self.tmpdir+"/_crt"+str(os.getpid())+rr
                write_to_file(crt_file, correction+'\n')
                opt_file = self.tmpdir+"/_opt"+str(os.getpid())+rr
...
#Target-Source		
		...
		self.aligner_t2s.stdout.flush()
                ...
                align_trg2src_file = self.tmpdir+"/_t2s"+str(os.getpid())+rr
...
class Aligner_GIZA:
	...
	"""
		...
		crt_file = self.tmpdir+"/_crt"+str(os.getpid())+rr
		write_to_file(crt_file, correction+'\n')
		src_file = self.tmpdir+"/_src"+str(os.getpid())+rr
